...
{
    return sqr(cow[u].first - cow[v].first) + sqr(cow[u].second - cow[v].second);
}
...
int main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = i + 1; j < N && j <= i + 190; j++)
        {
            ll d = distance(i, j);
            if (d <= 200)
                edges[d].push_back({i, j});
        }
    }

    for (int i = 0; i <= 200; i++)
        for (auto j : edges[i])
            if (merge(j.first, j.second))
                ans += i;
